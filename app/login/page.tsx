...
export default function Login(){
  ...
  return <main className="min-h-screen grid place-items-center p-5"><Card className="w-full max-w-md"><div className="text-center mb-7"><div className="text-2xl font-bold">ApplyWise AI</div><p className="text-sm text-gray-500 mt-1">AI-Powered Job Application & Resume Tailoring</p></div><form onSubmit={submit} className="space-y-4"><div><Label>Email</Label><Input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></div><div><Label>Password</Label><Input type="password" value={password} onChange={e=>setPassword(e.target.value)} required/></div>{error&&<p className="text-sm text-red-600">{error}</p>}<Button disabled={loading} className="w-full">{loading?"Signing in...":"Sign in"}</Button></form></Card></main>;
}
